...
/**
 * Represents the properties definition within a tool's function parameters.
 */
interface ToolPropertySpec {
  type: string;
  enum?: unknown[];
  items?: { type: string };
  [key: string]: any;
 ...
 *
 * @param {Tool} tool - The tool object containing the function specification.
 * @param {Record<string, any>} tool_call_content - The actual parameters passed to the tool function.
 * @returns {boolean} - Returns true if the tool call is valid.
 */
export function is_valid_tool_call(tool: Tool, tool_call_content: Record<string, any>): boolean {
  const props = (tool.function.parameters as any)?.properties as Record<string, ToolPropertySpec> | undefined;
  ...
  Object.entries(tool_call_content).forEach(([key, value]: [string, any]) => {
    if (!props![key]){
    ...
    if (Array.isArray(value) && props![key].type === 'array') {
      ...
      const itemType: string = typeof value[0];
      if (!value.every((item: any) => typeof item === itemType)){
  ...
  const required = (tool.function.parameters as any)?.required as string[] | undefined;

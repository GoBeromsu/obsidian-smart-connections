...
export class GeminiEmbedModelAdapter extends SmartEmbedModelApiAdapter {
  ...
  backoff_wait_time: number = 5000; // initial backoff wait time in ms
   ...
   */
  get req_adapter(): typeof SmartEmbedGeminiRequestAdapter {
   ...
   */
  get res_adapter(): typeof SmartEmbedGeminiResponseAdapter {
    return SmartEmbedGeminiResponseAdapter;
  }

  /** @returns {Record<string, SettingsConfigEntry>} Settings configuration for Gemini adapter */
  ...
  // no usage stats from Gemini so need to estimate tokens
  async embed_batch(inputs: EmbedInput[], retries: number = 0): Promise<EmbedInput[]> {
    if((globalThis as any).smart_env?.smart_sources?.entities_vector_adapter?.is_queue_halted) {

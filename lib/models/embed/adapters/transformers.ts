...
import type { AdapterDefaults, EmbedInput, ModelInfo, SettingsConfigEntry } from '../../types';
 ...
 */
export const transformers_defaults: AdapterDefaults = {
...
const is_webgpu_available = async (): Promise<boolean> => {
  // API exposed?
...
export class SmartEmbedTransformersAdapter extends SmartEmbedAdapter {
   ...
   * @returns {Promise<void>}
   */
  async load_transformers_with_fallback(): Promise<void> {
    ...
    const try_create = async (config_key: string): Promise<any> => {
      const pipe = await pipeline('feature-extraction', this.model_key!, DEVICE_CONFIGS[config_key] as any);
      return pipe;
    };
   ...
   * @param {EmbedInput[]} inputs
   * @returns {Promise<EmbedInput[]>}
   */
   ...
   * @private
   * @param {EmbedInput[]} batch_inputs
   * @returns {Promise<EmbedInput[]>}
   */
  async _process_batch(batch_inputs: EmbedInput[]): Promise<EmbedInput[]> {
    ...
    try {
      ...
      return batch_inputs.map((item, i) => {
        const vec = Array.from(resp[i].data).map((val: number) => Math.round(val * 1e8) / 1e8);
        item.vec = vec;
   ...
   */
  async _prepare_input(embed_input: string): Promise<{ text: string; tokens: number }> {
   ...
   * @returns {Promise<EmbedInput[]>}
   */
  async _retry_items_individually(batch_inputs: EmbedInput[]): Promise<EmbedInput[]> {
   ...
   * @returns {Promise<void>}
   */
  async _reset_pipeline_after_error(): Promise<void> {
    try {
      if (this.pipeline) {
        if (typeof this.pipeline.destroy === 'function') {
          this.pipeline.destroy();
   ...
   * @returns {Record<string, SettingsConfigEntry>}
   */
...
export const transformers_models: Record<string, ModelInfo> = {
 ...
 */
export const transformers_settings_config: Record<string, SettingsConfigEntry> = {
...
// 2025-11-26
export const settings_config: Record<string, SettingsConfigEntry> = {
  // "legacy_transformers": {
  //   name: 'Legacy transformers (no GPU)',
  //   type: "toggle",
  //   description: "Use legacy transformers (v2) instead of v3. This may resolve issues if the local embedding isn't working.",

...
export class SmartEnv extends BaseSmartEnv {
  ...
  async load(force_load = false) {
    ...
    this.refresh_status();
    const variableAdapter = this._config?.collections?.smart_completions
      ?.completion_adapters?.SmartCompletionVariableAdapter;
    if (variableAdapter?.register) {
      register_completion_variable_adapter_replacements(variableAdapter);
    } else {
      console.warn('SmartCompletionVariableAdapter not available');
    }
  }

import { SmartFsAdapterBase } from "./_base.js";

/**
 * SmartFsObsidianAdapter class
 *
 * This class provides an adapter for the Obsidian file system to work with SmartFs.
 * It wraps Obsidian's vault adapter methods and provides additional functionality
 * for path resolution and file operations.
 *
 * @class
 * @classdesc Adapter for Obsidian file system operations compatible with SmartFs
 */
export class SmartFsObsidianAdapter extends SmartFsAdapterBase {
  /**
  ...
  /**
   * Ensure a relative path has the full fs_path prefix
   * @param {string} rel_path - The relative path
   * @returns {string} The full path with fs_path prefix
   */
  _ensure_full_path(rel_path) {
    if (rel_path.startsWith(this.fs_path)) return rel_path;
    return this.fs_path + "/" + rel_path;
  }

  get_file(file_path) {
    const file = this._build_file_meta(file_path);
  ...
  async append(rel_path, data) {
    rel_path = this._ensure_full_path(rel_path);
  ...
  /**
   * Normalize path for list operations (handles leading/trailing slashes)
   * @param {string} rel_path - The relative path
   * @returns {string} The normalized path
   */
  _normalize_list_path(rel_path) {
    rel_path = this._ensure_full_path(rel_path);
  ...
  async list(rel_path, opts = {}) {
    rel_path = this._normalize_list_path(rel_path);
   ...
   * Overrides base implementation for Obsidian-specific handling
  ...
  async rename(old_path, new_path) {
    old_path = this._ensure_full_path(old_path);
    new_path = this._ensure_full_path(new_path);

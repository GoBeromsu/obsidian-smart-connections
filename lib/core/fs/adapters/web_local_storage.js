 ...
 */
export class WebLocalStorageFsAdapter extends SmartFsAdapterBase {
  /**
  ...
  get_file(file_path) {
    ...
    const is_folder = this.files[norm_path] === '[DIRECTORY]';
    const file = {
      path: norm_path,
      type: is_folder ? 'folder' : 'file',
      name: this._parse_name(norm_path)
    };
    if (!is_folder) {
      file.extension = this._parse_extension(norm_path);
      file.basename = this._parse_basename(file.name);
    }

 ...
 */
export class NodeFsSmartFsAdapter extends SmartFsAdapterBase {
  ...
  #wrap_method(method, path_count = 1) {
    return async (...args) => {
      ...
      const resolved_paths = paths.map(p => this.#resolve_path(p));
   ...
   * Overrides base implementation using Node's recursive readdir
  ...
  async list_recursive(rel_path, opts={}) {
    return await this.list(rel_path, { ...opts, recursive: true });
  }
